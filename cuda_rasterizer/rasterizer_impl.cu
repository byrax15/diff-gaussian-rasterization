...
static inline void CUDA_SYNC_CHECK_ALWAYS()
{
    cudaDeviceSynchronize();
    if (cudaPeekAtLastError() != cudaSuccess) {
        const auto e = cudaGetErrorString(cudaGetLastError());
        std::cerr << e << '\n';
        throw CudaRasterizer::render_error(e);
    }
}

static inline void CUDA_SYNC_CHECK()
{
#ifndef NDEBUG
...
CudaRasterizer::GeometryState::UniqueGeometryState
CudaRasterizer::GeometryState::fromBuffers(size_t P)
{
    UniqueGeometryState ug { new GeometryState() };
    auto& geom = *ug;

    cudaMalloc(&geom.depths, P * sizeof(float));
    cudaMalloc(&geom.clamped, P * 3 * sizeof(bool));
    cudaMalloc(&geom.internal_radii, P * sizeof(int));
    cudaMalloc(&geom.means2D, P * sizeof(float2));
    cudaMalloc(&geom.cov3D, P * 6 * sizeof(float));
    cudaMalloc(&geom.conic_opacity, P * sizeof(float4));
    cudaMalloc(&geom.rgb, P * 3 * sizeof(float));
    cudaMalloc(&geom.tiles_touched, P * sizeof(uint32_t));
    cub::DeviceScan::InclusiveSum(nullptr, geom.scan_size, geom.tiles_touched, geom.tiles_touched, P);
...
#else
    auto uniqueGeom = GeometryState::fromBuffers(P);
    auto& geomState = *uniqueGeom;
#endif
