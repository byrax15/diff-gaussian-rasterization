...
/**
 * Adapted from https://gradient-scaling.github.io/
 * @inproceedings {
 *      10.2312:sr.20231122,
 ...
 * }
 */
__global__ void scaleGradientsCUDA(
    int P, const float3* means3D, const float* viewmatrix,
    glm::vec3* dL_dmean2D, float* dL_dconic, glm::vec3* dL_dmean3D, float* dL_dcolor, float* dL_dcov3D, float* dL_dsh, glm::vec3* dL_dscale, glm::vec4* dL_drot)
{
    const auto idx = cg::this_grid().thread_rank();
    if (idx >= P)
        return;
    const auto position = [&] {
        auto p = transformPoint4x3(means3D[idx], viewmatrix);
        return *reinterpret_cast<glm::vec3*>(&p);
    }();
    const auto distance_coeff = min(max(glm::dot(position, position), 0.f), 1.f);
    dL_dmean2D[idx] *= distance_coeff;
    dL_dconic[idx] *= distance_coeff;
    dL_dmean3D[idx] *= distance_coeff;
    dL_dcolor[idx] *= distance_coeff;
    dL_dcov3D[idx] *= distance_coeff;
    dL_dsh[idx] *= distance_coeff;
    dL_dscale[idx] *= distance_coeff;
    dL_drot[idx] *= distance_coeff;
}
    ...
    glm::vec4* dL_drot)
{
    ...
    scaleGradientsCUDA<<<(P + 255) / 256, 256>>>(P, means3D, viewmatrix,
        reinterpret_cast<glm::vec3*>(dL_dmean2D), dL_dconic, dL_dmean3D, dL_dcolor, dL_dcov3D, dL_dsh, dL_dscale, dL_drot);
}
